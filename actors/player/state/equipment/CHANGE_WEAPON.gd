...
func _change_weapon_handle(nextWeapon: int) -> void:
	...
	var previousWeaponNode: Spatial = player.get_weapon_node(player.previousWeapon, player.hand)
	var currentWeaponNode: Spatial = player.get_weapon_node(player.currentWeapon, player.hand)
